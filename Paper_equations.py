...
def paper_differential_equations(y, t, m, q_0):
    ...
    der_theta = - ((1-np.sqrt(2*psi)*np.cos(theta))*(P_J + psi_p))**2/np.sqrt(2*psi)*np.cos(theta) - m*np.cos(theta)/np.sqrt(2*psi) + (1 - np.sqrt(2*psi)*np.cos(theta))**2*(P_J +psi_p)/q
...
# alternative Runge-Kutta function
